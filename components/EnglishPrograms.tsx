export default function EnglishPrograms() {
  ...
  return (
    <section id="english" className="py-16 md:py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-white to-blue-50 dark:from-transparent dark:to-blue-900/10 transition-colors">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">English Programs</h2>
          <p className="text-gray-600 dark:text-gray-400 text-lg">Special focus on English mastery</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {programs.map((program, index) => (
            <div key={index} className="bg-white dark:bg-gray-800 border dark:border-gray-700/50 rounded-xl shadow-md hover:shadow-lg transition-shadow p-8">
              ...
              <ul className="space-y-3">
                {program.details.map((detail, i) => (
                  <li key={i} className="flex items-center text-gray-700 dark:text-gray-300">
                    <span className="text-teal-500 dark:text-teal-400 mr-3 font-bold">✓</span>
                    {detail}
                  </li>
                ))}
              </ul>
